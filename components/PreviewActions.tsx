...
const ActionButton: React.FC<{
  ...
  <button
    onClick={onClick}
    disabled={isBusy}
    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-[var(--color-text-secondary)] bg-white/5 rounded-lg border border-transparent hover:bg-white/10 hover:text-[var(--color-text-primary)] transition-all disabled:opacity-50"
  >
    {icon}
    {label}
  </button>
...
const PreviewActions: React.FC<PreviewActionsProps> = ({ variant, onSaveStyleDna, onCopyCode, onShowToast }) => {
    ...
    return (
        <div className="flex-shrink-0 glass-panel rounded-2xl p-2">
            <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1 p-1 bg-black/10 rounded-xl border border-white/5">
                   ...
                   <ActionButton 
                        icon={justCopied ? <Checkmark className="w-4 h-4 text-green-500"/> : <Copy className="w-4 h-4" />} 
            ...
            {isCodeVisible && (
              <div className="mt-2 border-t border-[var(--color-border)] pt-2">
                <pre className="bg-black/50 p-4 text-sm text-sky-300 rounded-lg overflow-auto max-h-48 chat-scrollbar font-['JetBrains_Mono'] border border-[var(--color-border)]">
